...
5. **Pipelines (optional, after Hermes proves stable)**
   ...
   - [ ] **Invoice backlog drain (~811 PDF-bearing + 159 image-only).** Staged script: `scripts/invoice-backlog-drain.sh` (dry-run by default; `--execute` to run; it counts live). Drains invoices stuck `status='new'` since the ~2026-05-30 u125 cron outage — resets the `no_pdf` stamp, re-fetches PDFs (u125), re-extracts (u35), paced for budget. Deferred to the 48GB card (re-runs the ladder + vision OCR at volume). NOTE 2026-06-15: the routine `u125` cron is already restored, and the ~1,260 classifier-noise rows (non-invoices) were bulk-triaged to `ignored` (tag `bulk-triaged-noise-20260615`, reversible) — so the real recoverable set is ~811 with PDFs + 159 image-only. The 159 ('haiku_no_text') need the vision path — verify `u281-vision-ocr-drain` is *accepting* on the new card (logged accepted=0/rejected=29 on the 3060) before resetting those. Plus ~84 portal-notification invoices (doc in Xero/Dojo) for manual/portal reconciliation. Diagnosis: git 2026-06-15 + drain script header.
