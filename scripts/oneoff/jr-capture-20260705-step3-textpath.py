#!/usr/bin/env python3
"""jr-capture-20260705-step3text-step2b-extract.py — one-off re-extraction of the 19 J&R
...
Run:
  python3 scripts/oneoff/jr-capture-20260705-step3text-step2b-extract.py           # dry
  python3 scripts/oneoff/jr-capture-20260705-step3text-step2b-extract.py apply    # write
"""
import json
import re
import subprocess
...
IDS = [13182, 13194, 13427, 13486, 13656, 13716, 13729, 13737, 13776, 13879,
       13894, 14176, 14181, 14217, 14269, 14498, 14618, 14938, 15118, 15137, 15150]
            ...
            ok_write = psql_exec(f"""
                UPDATE vendor_invoice_inbox
                   SET net_amount={n}, vat_amount={v}, gross_amount={g},
                       ...
                       extracted_at=now(), pipeline_version='jr-capture-20260705-step3text',
                       status='extracted',
                       notes = left(coalesce(notes,'')||' [jr-capture-20260705-step3text:text-path-reextract]', 500)
