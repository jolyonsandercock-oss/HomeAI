...
INSERT INTO ops.pipeline_registry (name, kind, script_path, schedule_cron, target_rel, freshness_sql, freshness_sla_hours, notes) VALUES
 ...
 ('tanda_timesheets','sync','scripts/u47-tanda-timesheets-sync.sh','20 2 * * *','workforce_timesheets',
    'SELECT max(created_at) FROM workforce_timesheets',30,'timesheets'),
 ...
 ('invoice_pdf_attach_fetch','sweep','scripts/u125-pdf-attachment-fetch.sh','5 * * * *','email_attachments',
    'SELECT max(created_at) FROM email_attachments',6,'fetch PDFs for new invoices'),
