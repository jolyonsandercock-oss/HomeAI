...
while :; do
  ...
  if [ "$(PG -c "select (value ? 'paused_at') from static_context where key='system.state';" 2>/dev/null)" = "t" ]; then
    PG -c "update static_context set value=(value-'paused_at'-'paused_reason')||'{\"state\":\"running\"}'::jsonb where key='system.state';" >/dev/null 2>&1
    echo "$(ts) guardrail: cleared auto-pause" >> "$LOG"
  fi
  resp=$(curl -s -m 600 -X POST -H 'X-Realm: owner' "$BASE/api/memory/distill-batch?limit=$BATCH" 2>/dev/null)
  ...
  total_dossiers=$(PG -c "select count(*) from counterparty_dossier;" 2>/dev/null)
