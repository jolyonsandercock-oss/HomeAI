...
N8N_KEY=$(docker exec -e VAULT_TOKEN="$VAULT_TOKEN" homeai-vault vault kv get -field=api_key secret/n8n 2>/dev/null) || N8N_KEY=""
...
" >> "$LOG" 2>&1 || echo "  n8n export failed" >> "$LOG"
fi
...
LAST_REKEY=$(docker exec homeai-postgres psql -U postgres -d homeai -tAc \
  "SELECT value->>'last_rekey_date' FROM static_context WHERE key='vault.rekey'" 2>/dev/null) || LAST_REKEY=""
TODAY=$(date +%F)
...
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW();" >> "$LOG" 2>&1 || echo "  vault.rekey baseline insert failed" >> "$LOG"
else
...
" >> "$LOG" 2>&1 || echo "  vault rekey reminder send failed" >> "$LOG"
  fi
fi
...
restic snapshots --compact 2>&1 | tail -3 >> "$LOG" || echo "  restic snapshot summary failed" >> "$LOG"
...
req = urllib.request.Request(f\"https://api.telegram.org/bot{d['bot_token']}/sendMessage\",
    data=urllib.parse.urlencode({'chat_id': d['chat_id'], 'text': text}).encode())
urllib.request.urlopen(req, timeout=10)
" >> "$LOG" 2>&1 || echo "  completion telegram send failed" >> "$LOG"
