...
def load_snapshot_draft() -> dict | None:
    ...
    py = (
        ...
        "# the claude.ai Gmail connector is signed in as jolyboxbot, so the cloud\n"
        "# routine's draft lands in the BOT's drafts; check bot first, jo as backup\n"
        "acct, msgs = None, []\n"
        "for a in ('bot', 'jo'):\n"
        "    q = urllib.parse.urlencode({'account':a,'q':'in:draft subject:review-sweep-snapshot','max_results':5})\n"
        "    m = json.loads(urllib.request.urlopen('http://google-fetch:8011/messages?'+q, timeout=30).read())\n"
        "    m = m.get('messages', m) or []\n"
        "    if m: acct, msgs = a, m; break\n"
        "if not msgs: print('NODRAFT'); sys.exit(0)\n"
        "newest = max(msgs, key=lambda m: int(m.get('internal_date') or 0))\n"
        "full = json.loads(urllib.request.urlopen(f\"http://google-fetch:8011/message/{acct}/{newest['id']}\", timeout=30).read())\n"
        "def body_text(p):\n"
